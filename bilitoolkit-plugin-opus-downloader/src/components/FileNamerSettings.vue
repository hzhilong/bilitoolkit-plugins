...
import { testOpusDetail } from '@/test/opus-detail'
import { type DownloadResourceType, downloadResourceTypeMap } from '@/types/download'
...
const resetDefault = async () => {
  await showConfirm('是否恢复默认的文件命名模板？')
  await reset()
  initCurrFields()
}
...
const updateExampleList = () => {
  const types: DownloadResourceType[] = ['md']
  const list = types.map((type: DownloadResourceType) => ({
    extension: `.${type}`,
    name: downloadResourceTypeMap[type],
    namingResult: createFileNamer(settings.value).resolve(testOpusDetail),
  }))
  exampleDir.value = list[0].namingResult.dir
  exampleList.splice(
    0,
    exampleList.length,
    ...list.map(({ extension, name, namingResult }) => ({
      name: name,
      fileName: `${namingResult.fileName}${extension}`,
    })),
  )
}
...
<template>
  <div class="file-namer-settings">
    ...
    <div class="form-item">
      ...
      <div class="value-section fields curr-fields">
    ...
    <div class="form-item example-list">
      ...
      <div class="value-section">
        ...
        <div class="item" v-for="item in exampleList" :key="item.name">
